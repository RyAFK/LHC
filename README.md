...
- **Scroll-reveal animations** (`components/ui/Reveal.tsx`, `hooks/useInView.ts`) fade and lift section headings and grid cards (16px on one axis, ~650ms, the brief's `cubic-bezier(.22,1,.36,1)`) as they enter the viewport via `IntersectionObserver` — no scroll-position math, no library. Each reveal fires once and never re-hides on scroll-up. `hooks/usePrefersReducedMotion.ts` (shared with the hero) disables it entirely under `prefers-reduced-motion: reduce`, rendering content already-visible with no transition. Deliberately **not** applied to the emergency notice or the final booking CTA — safety and conversion content should never wait on a scroll trigger to appear.
